...
def analysis_selected_dataframe(selected_df: pd.DataFrame) -> pd.DataFrame:
    ...
    ls_cols = ["remip", "logid", "action", "reason", "desc"]
...
# error_df = load_data('/LogAnalysis/LogAnalysis_20240527/Monthly_error_Logs_Data_202405.csv')
error_df = load_data('/root/PythonAutomatic/LogAnalysis/Monthly_error_Logs_Data_202405.csv')
error_df.drop(columns=[
    'devid',
    'eventtime',
    'tz',
    'devname'
...
st.text('全量データに基づいて、remip, logid, action, reason, descという５つのアイテムを統計した後に表示されます')
analysis_df = analysis_selected_dataframe(error_df)
